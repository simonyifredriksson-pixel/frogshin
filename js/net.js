...
import { CFG, BUILD } from './config.js?v=v158';
import { roomCode as makeRoomCode } from './util.js?v=v158';
import { ECLIPSE_TITLE, CHAMPION_MARK } from './skins.js?v=v158';
...
export class Network {
  ...
  /**
   * The send itself. Split out so the background tick in main.js can push a
   * packet without going through the frame-rate accumulator — see
   * `Game._backgroundTick`.
   */
  _pushState(getState) {
    if (!this.isOnline || !this.connected) return;
    const s = getState();
